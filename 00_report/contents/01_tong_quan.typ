...
+ *Định nghĩa trạng thái mong muốn (Desired State):* Người quản trị thiết lập các thông số cấu hình mạng mới thông qua giao diện đồ họa. Dữ liệu này được kiểm tra tính hợp lệ (Validation) và lưu ở trạng thái chờ (Pending).
+ *Xem trước và thực thi (View \& Push):* Sử dụng engine Jinja2 để kết xuất dữ liệu chờ thành các tập lệnh CLI chuẩn xác. Người quản trị được quyền kiểm duyệt (Preview) mã lệnh trước khi các tiến trình nền đẩy lệnh xuống thiết bị.
...
Để đạt được mục tiêu tổng quát trên, đề tài xác định năm nhóm mục tiêu cụ thể sau: xây dựng giao diện đồ họa hiện đại, trực quan bằng Qt Quick/QML kết hợp cầu nối PyQt6, bảo đảm luồng giao diện chính luôn mượt mà và không bị nghẽn khi thực thi các tác vụ mạng nền; thiết kế cơ sở dữ liệu SQLite cục bộ phục vụ lưu trữ cấu hình mong muốn và dữ liệu giám sát, tích hợp thư viện Dulwich để quản lý lịch sử sao lưu cấu hình theo chuẩn Git; hoàn thiện luồng tự động hóa View \& Push cho các chức năng cốt lõi gồm định tuyến (Static Route, OSPF, EIGRP), dịch vụ IP (DHCP, NAT/PAT, FHRP) và chuyển mạch cùng an ninh Lớp 2 (VLAN, Trunking, EtherChannel, STP, VTP, Standard/Extended/Dynamic/Reflexive ACL, Port Security, DHCP Snooping, DAI); tích hợp các tiện ích vận hành và giám sát gồm máy chủ Syslog thu thập nhật ký thời gian thực, máy khách SFTP truyền tệp an toàn và một terminal nhúng phục vụ thao tác dòng lệnh trực tiếp; và cuối cùng bảo đảm an toàn hệ thống thông qua cơ chế khóa theo thiết bị (Host Lock) chống xung đột lệnh và đóng gói dự án an toàn với mã hóa Argon2id kết hợp AES-256-GCM.
...
Để thực hiện đề tài, nhóm tác giả áp dụng kết hợp các phương pháp sau. Trước hết là phương pháp khảo sát và phân tích nghiệp vụ, khảo sát thực tế các bài thực hành mạng tại Học viện, phân tích cú pháp CLI, quy trình cấu hình và các lỗi thường gặp khi thao tác trên Cisco IOS. Tiếp theo, phương pháp thiết kế hướng module  được sử dụng để phân tách hệ thống thành bốn tầng rõ ràng — giao diện, cầu nối, dữ liệu và mạng — bảo đảm tính độc lập, dễ bảo trì và mở rộng. Phương pháp mô hình hóa dữ liệu quan hệ được áp dụng để chuẩn hóa lược đồ cơ sở dữ liệu SQLite thành các nhóm bảng quản lý trạng thái mong muốn (Desired State) và dữ liệu thu thập (Observed State). Phương pháp kiểm thử tự động được sử dụng để xây dựng bộ test suite đa tầng gồm Unit Test, Integration Test, Contract Test và Smoke Test cho giao diện. Cuối cùng, phương pháp thực nghiệm và đánh giá so sánh được triển khai bằng cách thiết lập các topo mạng thực tế trên môi trường mô phỏng EVE-NG để kiểm chứng tính đúng đắn của cấu hình sinh ra, đo đạc thời gian thực thi, tỷ lệ thành công và so sánh định lượng với phương pháp cấu hình thủ công qua CLI.
