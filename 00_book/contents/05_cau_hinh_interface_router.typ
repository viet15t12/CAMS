...
// Nguồn nội dung: DOC/05_cau_hinh_interface_router.md
#import "../config/images.typ": insert-image
#import "../config/tables.typ": report-table

= Xem và cấu hình Interface trên Router <ch05>

Chương này tiếp tục từ phần quản lý thiết bị: R1 đã có trong Workspace và dữ liệu interface vật lý đã được lấy từ thiết bị bằng *Connect*, *Sync* hoặc *Get running-config*. Mục tiêu là xem danh sách interface, sửa interface vật lý, tạo interface ảo và kiểm tra lệnh trước khi Push.

Các hình được chụp từ giao diện CAMS thật với dữ liệu lab cố định. Ví dụ dùng R1 (#raw("192.0.2.1"), Cisco IOS); địa chỉ cấu hình bên trong form chỉ là địa chỉ mẫu dành cho tài liệu.
...
- Thanh tab con *Physical*, *Loopback*, *Tunnel* và *Subinterface* dùng để chọn loại interface.
...
#report-note[Nút *Reload UI* chỉ đọc lại dữ liệu từ database của CAMS. Nó không tự lấy running-config mới từ router. Khi danh sách Physical trống, hãy quay về Devices Sidebar, bảo đảm thiết bị đã kết nối rồi thực hiện *Sync* hoặc *Get running-config*.]

== Xem interface vật lý đã đồng bộ

Tên interface Physical do dữ liệu đồng bộ từ thiết bị cung cấp. CAMS không cho gõ tên mới hoặc xóa một cổng vật lý từ màn hình này. Chọn biểu tượng sửa bên phải #raw("GigabitEthernet0/0") để nạp dữ liệu vào form.

Phần *Identity and addressing* hiển thị tên interface, profile, địa chỉ IPv4, subnet mask, mô tả và trạng thái administratively down. Với interface vật lý, profile *L3* cung cấp thêm các tùy chọn Layer 3; profile *WAN* dùng cho các tham số PPP, PPPoE, HDLC hoặc Frame Relay.
...
#report-table(
  ...
  caption: [Chỉnh cấu hình Physical],
  figure-label: <tab:ch05-table-1>,
)
...
+ Host đích đúng là router cần cấu hình.
+ Tên interface, địa chỉ IP và subnet mask đúng với thiết kế.
+ Lệnh #raw("shutdown") hoặc #raw("no shutdown") đúng với trạng thái mong muốn.
+ VLAN ID, tunnel source/destination và các lệnh xóa không gây ảnh hưởng ngoài phạm vi dự kiến.

Chọn *Refresh* nếu đã sửa desired state sau khi mở hộp preview. Chỉ chọn *Push* khi đã có bản sao running-config phù hợp và phiên SSH/Telnet tới thiết bị đang sẵn sàng. Push hiện hỗ trợ Router Interface trên Cisco IOS qua SSH/Telnet; RESTCONF/NETCONF, IPv6, verify sau Push và rollback tự động chưa được tích hợp đầy đủ.

== Bài thực hành ngắn

+ Kết nối hoặc Sync R1, sau đó mở *Interface → Physical*.
+ Chọn một interface vật lý và đối chiếu tên, IP, mask với running-config.
+ Thay đổi một mô tả an toàn trong môi trường lab và chọn *Update Interface*.
+ Tạo #raw("Loopback0") với một địa chỉ /32 dành cho lab.
+ Nếu topology có trunk router-on-a-stick, tạo một Subinterface với VLAN ID phù hợp; nếu không, bỏ qua bước này.
+ Mở *View & Push*, đọc toàn bộ lệnh và đóng hộp thoại mà chưa Push.
+ Chỉ trong môi trường lab được phép thay đổi, mở lại preview và Push; sau đó lấy running-config mới để xác minh.
