...
#report-note[*Lưu ý:* Trong CAMS hiện tại, *Disconnect connected* đưa host về *Waiting*. *Reconnect* cũng chỉ đưa host từ Disconnected về Waiting, chưa mở phiên mới. Lưu chỉnh sửa thiết bị đóng phiên hiện có và đưa thiết bị về Waiting. Vì vậy không nên suy ra lịch sử kết nối chỉ từ tên nhóm.]

Thiết bị Waiting chưa cho mở vùng cấu hình bằng thao tác chọn đơn. Với thiết bị khả dụng, chọn dòng trong Sidebar để mở hoặc chuyển tới tab. Đóng tab chỉ đóng vùng làm việc; việc ngắt phiên và xóa host dùng các lệnh riêng bên dưới.
...
#report-note[*Giới hạn hiện tại:* Form cho phép khai báo cả bốn giao thức, nhưng luồng phiên CLI và Get running-config trong chương này hỗ trợ *SSH/TELNET*. Khai báo NETCONF hoặc RESTCONF không có nghĩa các lệnh kết nối và cấu hình qua CLI sẽ hoạt động.]
...
Role mô tả cách CAMS tổ chức chức năng cho thiết bị; nó không chuyển một switch Layer 2 thành thiết bị Layer 3. Nếu nhóm chức năng không đúng, kiểm tra Role và tab đang active.
...
#report-note[*Cảnh báo:* Thuật toán SSH legacy có thể làm giảm mức an toàn của kết nối. Chỉ thêm ngoại lệ cần thiết cho đúng thiết bị và theo chính sách của hệ thống.]

*Close* trở về form; các thiết lập được lưu cùng thao tác Add Device hoặc Save Changes. *Reset to default* xóa các ngoại lệ; trong Edit, thao tác reset cập nhật ngay thiết lập đã lưu. *Test SSH* chỉ khả dụng khi sửa thiết bị đã có, lưu các giá trị đang nhập rồi thực hiện kiểm tra kết nối SSH. Chỉ dùng khi đã sẵn sàng truy cập thiết bị.

=== Lưu thiết bị

Kiểm tra lại Host, Protocol, Port, OS và Role rồi chọn *Add Device*. Nếu form báo lỗi, sửa trường được chỉ ra rồi thử lại. Host đã có trong inventory không thể thêm lần nữa; hãy dùng Edit nếu cần đổi thông tin.
...
#report-note[*Lưu ý:* Import ghi thiết bị trực tiếp vào inventory. Khi có thiết bị được thêm, cửa sổ đóng; không có bước nạp các dòng để duyệt lại trong bảng Batch. Kiểm tra tệp trước khi chọn, nhất là Host, cổng, vai trò và thông tin xác thực. Luồng import hiện không áp dụng đầy đủ các kiểm tra của form nhập tay.]
...
#report-note[*Giới hạn hiện tại:* Các lựa chọn DEVICE TYPE chưa khớp mã phân loại thiết bị trong inventory, nên chọn Router hoặc Switch có thể làm danh sách trống dù có thiết bị tương ứng. Bỏ chọn lọc loại và dùng Search theo tên/IP kết hợp STATUS.]
...
#report-note[*Ghi chú:* Chọn nhiều chỉ chọn các bản ghi trong inventory, chưa thực hiện thao tác mạng. Chỉ khi kích hoạt Connect, Get configs hoặc Disconnect thì tác vụ tương ứng mới bắt đầu.]
...
Trạng thái Connected không bảo đảm mọi bước thu thập, backup hoặc đồng bộ đã thành công: CAMS có thể giữ kết nối và báo cảnh báo cho một bước tiếp theo. Đọc thông báo chi tiết; không bấm Connect lặp lại khi tác vụ cũ còn chạy.
...
*Get running-config* đọc cấu hình đang hoạt động trên thiết bị. Với R1 Connected, chuột phải chọn lệnh này và chờ kết quả. Luồng hiện tại thu thập qua SSH/TELNET, lưu vào lịch sử backup của host, đồng thời áp dụng chính sách đồng bộ tự động cho các dữ liệu cấu hình được hỗ trợ.
...
*Save configuration* lưu cấu hình đang chạy thành cấu hình khởi động trên thiết bị qua driver của phiên quản lý hiện có. Thiết bị/driver phải hỗ trợ thao tác lưu; đọc kết quả để biết thành công hay bị từ chối. Đây là thao tác lên thiết bị và khác với *Save Workspace*, vốn lưu project CAMS.

*Sync* thu thập running-config mới để đối chiếu và cập nhật dữ liệu cấu hình được hỗ trợ trong Workspace. Sync không đẩy toàn bộ dữ liệu Workspace lên thiết bị. Nếu không có xung đột, CAMS áp dụng chế độ an toàn. Khi có thay đổi cục bộ đang chờ, hộp *Manual Sync conflict* cho chọn:

- *Keep pending changes*: bỏ qua các nhóm dữ liệu đang có thay đổi chờ, giữ phần chỉnh sửa cục bộ đó.
- *Use device state*: bỏ thay đổi cục bộ đang chờ và thay bằng trạng thái mới từ thiết bị.
- *Cancel*: hủy quyết định đồng bộ lần này.

#report-note[*Lưu ý:* Chỉ chọn *Use device state* khi đã kiểm tra và chấp nhận bỏ phần chỉnh sửa cục bộ liên quan. Kết quả backup thành công và kết quả đồng bộ là hai bước riêng; đọc cảnh báo nếu một bước chưa hoàn tất.]
...
SW3 vẫn nằm trong số bốn host đã chọn nhưng không thuộc mục tiêu của ba lệnh trên. Muốn kết nối SW3, trước hết dùng Reconnect để đưa host về Waiting. Lệnh có số lượng bằng không bị vô hiệu hóa.

Theo dõi tiến độ và kết quả từng host; lỗi của một host không có nghĩa toàn bộ nhóm đều thất bại. Khi batch hoàn tất, CAMS thông báo số thành công/thất bại, nạp lại danh sách và thoát chế độ chọn nhiều. Chọn lại nhóm nếu muốn chạy tác vụ tiếp theo.
...
Lưu chỉnh sửa đóng phiên quản lý hiện có và đưa thiết bị về Waiting. Sau khi kiểm tra thông tin mới, dùng Connect để mở lại phiên. Đừng hiểu trạng thái Waiting sau Edit là mất bản ghi.
