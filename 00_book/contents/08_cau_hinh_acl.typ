...
// Nguồn nội dung: DOC/08_cau_hinh_acl.md
#import "../config/images.typ": insert-image

= Cấu hình ACL trên Router <ch08>

Chương này trình bày các loại Access Control List trong tab *ACL* của CAMS: Standard, Extended, Dynamic, Reflexive, MAC và Bindings. ACL có cửa sổ *View & Push ACL* riêng, độc lập với DHCP và Routing.

#report-note[ACL được xử lý từ sequence nhỏ đến lớn, dừng tại rule khớp đầu tiên và có implicit deny ở cuối. Một rule sai có thể làm mất kết nối quản trị; luôn xem preview và bảo đảm có đường khôi phục trước khi Push.]

== Mở tab ACL

Chọn router rồi chọn *ACL* trên Feature Bar. Chọn loại ACL trên thanh tab con. Khung bên trái hiển thị thông tin ACL và Rule Builder; khung bên phải hiển thị ACL đã lưu cùng danh sách rule. *View* mở nội dung ở chế độ chỉ đọc, *Edit* cho phép thay đổi và *Delete* chỉ đánh dấu xóa cho đến khi Save.
...
Sau Push, kiểm tra running-config, ACL hit counter và thử cả lưu lượng được phép lẫn lưu lượng phải bị chặn. Nếu thay đổi ACL qua phiên SSH, nên có console hoặc đường quản trị dự phòng.
