...
// Nguồn nội dung: DOC/12_truyen_tep_sftp.md
#import "../config/images.typ": insert-image
#import "../config/tables.typ": report-table

= Truyền tệp bằng SFTP/SCP <ch12>

Workspace *SFTP/SCP* của CAMS dùng để truyền tệp qua SSH giữa máy chạy CAMS và máy chủ từ xa. SFTP phù hợp khi cần duyệt và quản lý thư mục remote; SCP phù hợp khi đã biết chính xác đường dẫn nguồn hoặc đích và chỉ cần truyền dữ liệu.

Mở *SFTP/SCP* trên Activity Bar hoặc nhấn #raw("Ctrl+Alt+F"). Cả hai giao thức dùng SSH và thường dùng cổng 22, nhưng cách thao tác với filesystem remote khác nhau. CAMS không cung cấp FTP không mã hóa trong workspace này.

== Chọn SFTP hay SCP

#report-table(
  columns: (1fr, 1.4fr, 2.4fr),
  header: ([Chế độ], [Nên dùng khi], [Khả năng trong CAMS]),
  rows: (
  ...
  caption: [Chọn SFTP hay SCP],
  figure-label: <tab:ch12-table-1>,
)

Việc chọn SCP không làm panel remote hoạt động như SFTP. Nếu cần xem nội dung thư mục trước khi chọn tệp, dùng SFTP.
...
#report-table(
  ...
  rows: (
    ([*Display name*], [Tên gợi nhớ hiển thị trong danh sách kết nối, ví dụ #raw("Backup Server").]),
    ([*Transfer mode*], [Chọn #raw("SFTP") để duyệt remote hoặc #raw("SCP") để truyền theo đường dẫn cụ thể. Chế độ này được lưu cùng profile.]),
    ([*Host / IP*], [Tên miền hoặc địa chỉ IP của SSH server. Phải có đường mạng từ máy CAMS đến địa chỉ này.]),
    ([*Port*], [Cổng SSH của server; thường là #raw("22"). Nếu quản trị viên đổi cổng SSH, nhập đúng cổng đã được cấp.]),
    ...
    ([*Private key (optional)*], [Đường dẫn khóa riêng SSH, ví dụ #raw("/home/user/.ssh/id_ed25519"). Dùng nút *Browse* để chọn tệp, đồng thời bảo vệ quyền truy cập tệp khóa.]),
  ),
  caption: [Tạo kết nối SFTP/SCP],
  figure-label: <tab:ch12-table-2>,
)

Nên ưu tiên *private key* hoặc SSH agent thay cho lưu mật khẩu. Nếu CAMS báo #raw("Secure password storage is unavailable on this system"), tùy chọn lưu mật khẩu sẽ bị vô hiệu hóa; người dùng cần nhập mật khẩu khi kết nối hoặc dùng khóa SSH.
...
- Với *SFTP*, *Initial remote directory* là thư mục được mở sau khi kết nối, ví dụ #raw("/network/configs").
- Với *SCP*, trường này đổi thành *Remote upload/download path*. Nhập đường dẫn remote cụ thể, ví dụ #raw("/network/incoming/R1.cfg") hoặc thư mục đích được server cho phép.

Chọn *Save* để lưu profile. Trước khi dùng trên hệ thống thật, kiểm tra Transfer mode, Host/IP, username, khóa riêng và hai đường dẫn; không đưa mật khẩu hoặc khóa riêng vào tài liệu dùng chung.
...
- Nhập đường dẫn hoặc dùng *Back*, *Forward*, *Up*, *Refresh* để điều hướng.
- Nhấp đúp thư mục để mở. Chọn một hay nhiều tệp rồi nhấn *Upload* để đưa lên thư mục remote đang mở.
- *New folder*, *Rename* và *Delete* tác động lên phía local khi panel này đang được chọn. Kiểm tra kỹ mục được chọn trước thao tác xóa.
...
- *New folder*, *Rename* và *Delete* thực hiện trên server và phụ thuộc quyền của tài khoản SFTP.
- Trước khi truyền, kiểm tra đúng đường dẫn ở cả hai panel, tên tệp, dung lượng trống và quyền đọc/ghi. Với dữ liệu quan trọng, nên dùng tên có ngày giờ hoặc phiên bản để tránh nhầm bản backup.
...
SCP không có các thao tác duyệt, Refresh, New folder, Rename hoặc Delete ở phía remote. Nếu chưa chắc đường dẫn tồn tại, chuyển sang SFTP để kiểm tra hoặc xác minh bằng công cụ quản trị server.
...
- *Clear finished* xóa các mục đã hoàn thành khỏi hàng đợi, không xóa tệp đã truyền.

Không ngắt mạng hoặc đóng CAMS khi hàng đợi vẫn còn tác vụ đang chạy. Sau khi hoàn tất, làm mới panel đích và kiểm tra tên, kích thước tệp trước khi xem bản sao lưu là hợp lệ.
...
- *Running-config snapshot saved…* xác nhận bản cấu hình đã được lưu vào lịch sử của host.
- *Background synchronization completed…* xác nhận bước cập nhật database đã hoàn tất.
- Nếu backup thành công nhưng sync thất bại, snapshot vẫn có thể xem được; đọc cảnh báo và xử lý nguyên nhân đồng bộ riêng.
...
- Dòng bắt đầu bằng #raw("+") là nội dung được thêm vào bản mới.
- Các badge #raw("+N") và #raw("−N") cho biết tổng số dòng thêm/xóa; số *versions* cho biết khoảng lịch sử đang được so sánh.
- Nếu hai endpoint có nội dung giống nhau, vùng diff báo không có khác biệt.

#report-note[Diff chỉ dùng để xem và đối chiếu. Nó không tự khôi phục cấu hình cũ và không gửi lệnh lên thiết bị. Muốn quay lại Snapshot, chọn *Snapshot* rồi chọn Version cần xem.]

== Phím tắt thường dùng

#report-table(
  columns: (1.15fr, 2.85fr),
  header: ([Phím tắt], [Tác dụng trên panel đang hoạt động]),
  rows: (
    ([#raw("Alt+Left") / #raw("Backspace")], [Quay lại thư mục trước.]),
    ([#raw("Alt+Right")], [Đi tới thư mục kế tiếp trong lịch sử.]),
    ([#raw("Alt+Up")], [Mở thư mục cha.]),
    ([#raw("F5") / #raw("Ctrl+R")], [Làm mới danh sách.]),
    ([#raw("Ctrl+Shift+N")], [Tạo thư mục mới.]),
    ([#raw("F2")], [Đổi tên mục đang chọn.]),
    ([#raw("Delete")], [Xóa mục đang chọn sau bước xác nhận.]),
    ([#raw("Enter")], [Với SFTP, mở thư mục hoặc truyền mục đang chọn; với SCP, thực hiện theo đường dẫn remote đã nhập.]),
    ([#raw("Ctrl+A")], [Chọn tất cả mục trong panel.]),
    ([#raw("Esc")], [Bỏ vùng chọn.]),
    ([#raw("Shift+F10")], [Mở menu ngữ cảnh của tệp.]),
  ),
  caption: [Phím tắt thường dùng],
  figure-label: <tab:ch12-table-3>,
)
