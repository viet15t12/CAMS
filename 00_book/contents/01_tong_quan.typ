...
= Tổng quan về phần mềm CAMS <ch01>
...
CAMS lưu lịch sử cấu hình thiết bị để người dùng đọc lại và so sánh các phiên bản. Quy trình xem Snapshot và Diff được trình bày tại @ch12. Diff chỉ phục vụ đối chiếu, không tự khôi phục hoặc gửi lệnh lên thiết bị. Với trạng thái toàn project, dùng Snapshot và Roll Back theo @ch02; verify và rollback chưa được triển khai đầy đủ cho mọi luồng cấu hình.
...
CAMS tổ chức dữ liệu làm việc dưới dạng CAMS Project, với đuôi mở rộng của tệp là #raw(".ntp"). Người dùng có thể tạo, mở, lưu và quản lý các dự án mạng riêng biệt, giúp phân tách các môi trường làm việc khác nhau, sử dụng mật khẩu để bảo vệ tùy chọn và quản lý snapshot/rollback.
...
#flow-diagram(
  ...
  max-per-row: 3, node-height: 21mm,
)
...
Trước khi áp dụng lên thiết bị thực tế, kiểm tra giới hạn của từng chức năng trong chương tương ứng.
...
#report-table(
  ...
  rows: (
    ([Project], [Không gian làm việc được lưu dưới dạng package #raw(".ntp")]),
  ...
  caption: [Các khái niệm người dùng cần biết],
  figure-label: <tab:ch01-table-1>,
)
