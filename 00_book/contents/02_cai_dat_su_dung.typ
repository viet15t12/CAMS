...
= Cài đặt và Bắt đầu sử dụng CAMS <ch02>

Chương này hướng dẫn người dùng chạy CAMS từ mã nguồn, đi qua cửa sổ Welcome, tạo hoặc mở một project #raw(".ntp"), rồi xác nhận Workspace đã sẵn sàng. Chương này chưa yêu cầu thiết bị mạng thật và chưa hướng dẫn cấu hình router, switch hoặc dịch vụ mạng.
...
- Python #raw("3.11") trở lên;
- #raw("uv") để tạo môi trường Python và cài dependency từ #raw("pyproject.toml") cùng #raw("uv.lock");
- Git nếu lấy mã nguồn bằng lệnh #raw("git clone");
...
- Rust toolchain (#raw("cargo")): chỉ cần khi phải tự biên dịch CAMS Terminal companion;
...
*Mẹo:* Nếu mục tiêu trước mắt chỉ là mở ứng dụng và tạo Workspace, hãy chuẩn bị Python và #raw("uv") trước. Các thành phần tùy chọn có thể bổ sung sau.

== Lấy mã nguồn CAMS

Kho Git đang được sử dụng bởi project là #raw("viet15t12/CAMS"). Từ Terminal, PowerShell hoặc Command Prompt, chạy:
...
Root repository là thư mục làm việc của ứng dụng desktop. Các lệnh trong phần còn lại của chương được thực hiện tại đây.

Nếu nhận mã nguồn dưới dạng file nén thay vì Git, hãy giải nén toàn bộ repository, mở Terminal tại thư mục #raw("CAMS"), rồi tiếp tục với bước kiểm tra môi trường. Bản mã nguồn phải giữ nguyên các thư mục #raw("UI/"), #raw("core/"), #raw("features/"), #raw("infrastructure/") và file khóa dependency.
...
Kết quả phải là Python #raw("3.11") hoặc mới hơn, ví dụ #raw("Python 3.12.x").
...
Nếu Terminal hiển thị số phiên bản của #raw("uv"), có thể tiếp tục. Nếu hệ thống báo không nhận diện lệnh, hãy cài #raw("uv"), mở lại Terminal và kiểm tra lại trước khi chạy CAMS.
...
Sau đó mở *CAMS* từ menu ứng dụng hoặc chạy lệnh #raw("cams") ở bất kỳ thư mục nào. Bộ cài không cần #raw("sudo"), và việc chạy lại bộ cài để cập nhật không xóa database người dùng.
...
Khi chỉ cần chạy ứng dụng Python trong môi trường dependency đã được #raw("uv") chuẩn bị, dùng:
...
- *Create New*: tạo project #raw(".ntp") mới;
- *Open*: chọn một project #raw(".ntp") đã có;
...
Project CAMS là một package có phần mở rộng #raw(".ntp"). Package chứa dữ liệu Workspace cần thiết, gồm các database, dữ liệu backup và lịch sử snapshot. Mỗi project là một không gian làm việc độc lập; vì vậy nên dùng project riêng cho từng bài lab hoặc hệ thống.
...
Nên chọn tên mô tả đúng mục đích. Tránh các tên khó phân biệt như #raw("new.ntp") hoặc #raw("project1.ntp") khi quản lý nhiều môi trường.
...
+ Khởi chạy CAMS.
+ Trong cửa sổ Welcome, chọn *Create New*.
+ Trong hộp thoại *Create New Project*, nhập *Project name*.
+ Kiểm tra *Project location*. Có thể nhập đường dẫn thư mục hoặc chọn *Browse…* để dùng native folder picker.
+ Nếu muốn dùng thư mục này cho các project sau, bật tùy chọn đặt làm vị trí mặc định.
+ Tùy chọn, bật *Protect project with a password* và nhập mật khẩu hai lần.
+ Chọn *Create Project*.
+ Chờ hộp thoại đóng và Workspace xuất hiện.
...
Tùy chọn *Protect project with a password* bảo vệ toàn bộ package #raw(".ntp"). Khi mở lại project được bảo vệ, ứng dụng yêu cầu mật khẩu trước khi giải nén Workspace.
...
#report-note[*Cảnh báo:* Khi project đang mở, dữ liệu phải được giải nén để ứng dụng làm việc. Mật khẩu bảo vệ file #raw(".ntp") không thay thế việc bảo vệ tài khoản hệ điều hành, thư mục tạm và bản backup bên ngoài.]

Các ảnh của chương này dùng project không có mật khẩu. Không có password thật hoặc mẫu password nào xuất hiện trong PNG.

== Mở project đã có

Để mở một package #raw(".ntp"):

+ Trở về cửa sổ Welcome. Nếu đang ở Workspace, chọn *File* → *Close Workspace* và chờ thao tác lưu/đóng hoàn tất.
+ Chọn *Open* ở phần đầu cửa sổ như vị trí minh họa tại @fig:ch02-welcome.
+ Trong native file picker của hệ điều hành, chọn file #raw(".ntp").
+ Nếu project được bảo vệ, nhập mật khẩu và xác nhận.
+ Chờ CAMS kiểm tra package, giải nén dữ liệu và mở Workspace.
...
*Mẹo:* CAMS có cơ chế auto-save theo chu kỳ, nhưng nên chủ động dùng #raw("Ctrl+S") trước khi đóng ứng dụng hoặc trước một thay đổi quan trọng.

== Snapshot và khôi phục

Snapshot là điểm lưu trạng thái toàn project. Từ menu *File*:

+ Chọn *Create Snapshot…* để mở Snapshot History và đặt nhãn tùy chọn.
+ Chọn *Create Snapshot*.
+ Kiểm tra snapshot mới xuất hiện trong danh sách.
+ Chỉ dùng *Roll Back* khi đã xác định đúng mốc cần phục hồi.
...
Với bản cài Linux, dữ liệu runtime toàn cục mặc định nằm trong:

```text
~/.local/share/cams/data/
```

Đây là vị trí dữ liệu của bản cài Linux. Khi chạy trực tiếp từ mã nguồn, vị trí mặc định là #raw("CAMS/data/"). Thư mục này chứa database mặc định và trạng thái ứng dụng như Recent Projects. File project #raw(".ntp") nằm tại vị trí người dùng chọn trong Create/Open workflow.

Người dùng thông thường không cần đổi thư mục runtime. Trường hợp nâng cao có thể đặt biến môi trường #raw("CAMS_DATA_DIR") trước khi khởi chạy để dùng một vị trí khác. Hãy bảo đảm thư mục đích tồn tại trên ổ đĩa tin cậy và tài khoản hiện tại có quyền đọc/ghi.
...
Trạng thái sẵn sàng được minh họa tại @fig:ch02-created-workspace. Không cần tạo thiết bị hoặc kết nối mạng để hoàn thành bước kiểm tra này.
...
Kiểm tra lại #raw("python --version") hoặc #raw("python3 --version"). Bảo đảm phiên bản từ #raw("3.11") trở lên và executable đã có trong #raw("PATH").

=== uv không được nhận diện

Chạy lại #raw("uv --version") trong một Terminal mới. Nếu lệnh vẫn không tồn tại, cài #raw("uv") và kiểm tra cấu hình #raw("PATH") của tài khoản hiện tại.
...
Kiểm tra quyền ghi đối với thư mục dữ liệu ở trên hoặc thư mục do #raw("CAMS_DATA_DIR") chỉ định. Không chạy nhiều instance cùng ghi một Workspace. Không xóa file #raw("-wal"), #raw("-shm") hoặc journal khi ứng dụng còn chạy.

=== Terminal companion hoặc Syslog collector còn thiếu

Launcher đầy đủ có thể yêu cầu Rust/Cargo, CMake hoặc compiler để build thành phần native. Nếu chỉ cần hoàn thành chương này, có thể thử #raw("uv run main.py"); Terminal và Syslog native có thể chưa hoạt động. Chuẩn bị các binary đó trước khi dùng chức năng tương ứng.

== Lưu ý về an toàn dữ liệu

#report-note[*Quan trọng:* Không commit hoặc chia sẻ công khai project #raw(".ntp"), database, file #raw("-wal")/#raw("-shm")/journal, running-config, backup, Syslog, pcap, credential, password hoặc private key. Các file này có thể chứa topology, địa chỉ, cấu hình và thông tin xác thực của hệ thống thật.]

Ngoài snapshot trong project, hãy duy trì ít nhất một bản sao #raw(".ntp") ở vị trí độc lập trước khi thực hiện thay đổi lớn. Chỉ đóng, sao chép hoặc di chuyển project sau khi thao tác lưu đã hoàn tất. Không chỉnh trực tiếp database hoặc nội dung package #raw(".ntp") bằng công cụ ngoài khi Workspace đang mở.
