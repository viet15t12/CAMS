// Nguồn nội dung: DOC/03_giao_dien_dieu_huong.md
#import "../config/commands.typ": report-note
#import "../config/images.typ": insert-image
#import "../config/tables.typ": report-table

= Làm quen với giao diện và điều hướng CAMS <ch03>

Sau @ch02, CAMS đã khởi chạy, project #raw(".ntp") đã được tạo hoặc mở và Workspace đã sẵn sàng. Chương này giúp người dùng nhận biết vị trí các thành phần giao diện, chọn thiết bị, chuyển tab và tìm chức năng cần xem. Người dùng chưa cần cấu hình mạng hoặc kết nối tới thiết bị thật.

Các hình sử dụng Workspace minh họa *CAMS Interface Lab*, gồm R1 (#raw("192.0.2.1"), Router), R2 (#raw("192.0.2.2"), Router), SW1 (#raw("192.0.2.11"), Switch Layer 2) và SW3 (#raw("192.0.2.13"), Switch Layer 3). Đây là dữ liệu mẫu dành cho tài liệu. Trạng thái Connected của R1 và SW1 được mô phỏng an toàn, không phải kết quả kết nối tới một hệ thống mạng thật.
...
#report-table(
  ...
  caption: [Tổng quan Workspace],
  figure-label: <tab:ch03-workspace-regions>,
)
...
*File* tập hợp các lệnh liên quan tới project và phiên làm việc: *New Project…*, *Open Project…*, *Save Workspace*, *Create Snapshot…*, *Snapshot History…*, *Close Workspace* và *Quit*. Quy trình tạo, mở, lưu và snapshot đã được trình bày ở @ch02.
...
Để chọn nhiều dòng, giữ #raw("Ctrl") rồi nhấn từng thiết bị; giữ #raw("Shift") rồi nhấn để chọn một khoảng trong danh sách đang hiển thị. Khi chế độ chọn nhiều đang bật, nhấn một dòng sẽ thay đổi lựa chọn của nhóm thay vì chuyển tab. Thanh thao tác nhóm cho biết số thiết bị đã chọn; nhấn #raw("Esc") khi không nhập trong ô tìm kiếm để thoát lựa chọn nhóm.

Các lệnh quản lý thiết bị sẽ được trình bày ở @ch04. Trong chương này, chỉ cần phân biệt *chọn một thiết bị để xem* với *chọn nhiều thiết bị để thao tác theo nhóm*.
...
#report-table(
  ...
  rows: (
    ...
    ([Switch Layer 2 (#raw("sw2"))], [Switching, Security, Monitoring, Syslog Server]),
    ([Switch Layer 3 (#raw("sw3"))], [Routing, Switching, Services, Security, Monitoring, FHRP, Syslog Server]),
  ),
  caption: [Các chức năng theo loại thiết bị],
  figure-label: <tab:ch03-features-by-role>,
)
...
Các phím tắt sau tương ứng với các lệnh trong menu hiện tại. Với tổ hợp có dấu phẩy như #raw("Ctrl+K, Ctrl+S"), nhấn tổ hợp đầu, thả ra rồi nhấn tổ hợp sau.

#report-table(
  columns: (1fr, 1.4fr, 2.4fr),
  header: ([Phím tắt], [Lệnh], [Tác dụng]),
  rows: (
    ([#raw("Ctrl+O")], [Open Project], [Yêu cầu chuyển sang mở project.]),
    ([#raw("Ctrl+S")], [Save Workspace], [Lưu Workspace hiện tại.]),
    ([#raw("Ctrl+R")], [Reload UI], [Nạp lại nội dung giao diện đang xem.]),
    ([#raw("Ctrl+B")], [Toggle Sidebar], [Ẩn hoặc hiện Sidebar.]),
    ([#raw("Ctrl+Alt+D")], [Dashboard], [Trở về chế độ làm việc với thiết bị.]),
    ([#raw("Ctrl+Alt+F")], [SFTP], [Mở khu vực SFTP hoặc client đã chọn.]),
    ([#raw("Ctrl+Alt+L")], [System Logs], [Mở khu vực nhật ký hệ thống.]),
    ([#raw("Ctrl+Alt+B")], [Database], [Mở trình duyệt dữ liệu/công cụ đã chọn khi khả dụng.]),
    ([#raw("Ctrl+,")], [Settings], [Mở thiết lập ứng dụng.]),
    ([#raw("Ctrl+K, Ctrl+S")], [Keyboard Shortcuts], [Mở bảng tham khảo phím tắt.]),
  ),
  caption: [Các phím tắt điều hướng],
  figure-label: <tab:ch03-shortcuts>,
)
...
Với Device Tabs đang hiển thị, dùng #raw("Ctrl+Tab") để sang tab tiếp theo, #raw("Ctrl+Shift+Tab") để về tab trước, #raw("Ctrl+W") để đóng tab active và #raw("Ctrl+Shift+T") để mở lại tab vừa đóng. Các phím này giúp di chuyển nhanh mà vẫn giữ nguyên danh sách thiết bị trong project.

== Thực hành điều hướng cơ bản

Nếu có bộ project mẫu đi kèm tài liệu, bài tập sử dụng bản sao #raw("CAMS-Interface-Lab.ntp"), nằm trong thư mục #raw("book/fixtures/chapter-03/") của bộ tài liệu. Mở project mẫu theo cách đã biết ở @ch02, rồi bắt đầu từ Workspace đã sẵn sàng. Nếu chỉ có project trống, người dùng vẫn có thể luyện menu, Settings và ẩn/hiện Sidebar trước.
...
+ Chọn *View → Dashboard* và xác định Devices Sidebar, vùng nội dung cùng Status Bar.
+ Trong nhóm Disconnected của Workspace mẫu, chọn *R1*. Kiểm tra tab R1 xuất hiện và có dấu nhấn active như @fig:ch03-device-tabs.
+ Chọn biểu tượng *Information*. Đọc tên màn hình và địa chỉ #raw("192.0.2.1") ở đầu Content Area.
+ Chọn biểu tượng *Interface*, sau đó chọn *Routing*. Chỉ quan sát sự thay đổi màn hình, không nhập hoặc lưu cấu hình.
+ Chọn *SW1* ở Sidebar. Đối chiếu Feature Bar với @fig:ch03-feature-switch và nhận ra các nhóm Switching, Security, Monitoring.
+ Chuyển về tab *R1*. Kiểm tra tab nhớ Routing; chọn lại Information. Chọn R1 thêm lần nữa từ Sidebar và kiểm tra không có tab R1 trùng.
+ Nhấn #raw("Ctrl+B") để ẩn Sidebar, rồi nhấn lại để hiện. Kiểm tra host và feature đang xem không thay đổi.
+ Chọn *View → Settings*. Quan sát Sidebar chuyển sang danh mục thiết lập, sau đó trở về *View → Dashboard*.
+ Mở *Help → Keyboard Shortcuts*, tìm phím tắt Toggle Sidebar rồi đóng hộp thoại. Chỉ ra vị trí Status Bar và biểu tượng thông báo.
