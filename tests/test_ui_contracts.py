...
class ButtonIconContractTests(unittest.TestCase):
    ...
    def test_syslog_uses_current_workspace_table_and_resource_contracts(self) -> None:
        ...
        self.assertIn('title: "Reset log data"', settings)
        self.assertIn('objectName: "syslogResetScope"', settings)
        self.assertIn('objectName: "syslogResetExportButton"', settings)
        self.assertIn('objectName: "syslogResetConfirmationDialog"', settings)
        self.assertIn('text: "Permanently Delete"', settings)
        self.assertIn("DELETE ALL SYSLOG DATA", settings)
